...
process <- function(method){
    vGWAS <- rbind(read_delim(paste0("/mnt/project/DataAnalysis/05.OSCA/", trait, "/", trait, "_", method, "_arrayedSNPs")),
                   read_delim(paste0("/mnt/project/DataAnalysis/05.OSCA/", trait, "/", trait, "_", method, "_imputedSNPs"))) %>%
        mutate(genotype1=paste0(Chr,":",BP,"_",A1,"_",A2)) %>%
        mutate(genotype2=paste0(Chr,":",BP,"_",A2,"_",A1))
    snplist <- read_delim(paste0("/mnt/project/DataAnalysis/04.Regenie/",trait,"/",trait,".snplist"), col_names=F, delim="\t")

    vGWAS.geno1 <- vGWAS %>% filter(genotype1 %in% snplist$X1) %>% mutate(ID=genotype1) %>% select(-genotype1, -genotype2)
    vGWAS.geno2 <- vGWAS %>% filter(genotype2 %in% snplist$X1) %>% mutate(ID=genotype2) %>% select(-genotype1, -genotype2)
